#!/usr/bin/env python3
"""Generate GO Transit and TTC JSON station assets for DozeAlert."""

import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
TRANSIT_ROOT = ROOT / "assets" / "transit" / "Canada"
...
def write_line(country: str, system: str, line_name: str, stations: list[tuple]) -> None:
    system_dir = TRANSIT_ROOT / system.replace(" ", "_")
    ...
    payload = {
        ...
        "stations": [
            ...
            for index, (name, lat, lng) in enumerate(stations, start=1)
        ],
    }
    file_name = f"{line_name.replace(' ', '_')}.json"
    path = system_dir / file_name
    path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"Wrote {path.relative_to(ROOT)} ({len(stations)} stations)")


def main() -> None:
    for line_name, stations in GO_TRANSIT.items():
        write_line("Canada", "GO Transit", line_name, stations)

    for line_name, stations in TTC.items():
        write_line("Canada", "TTC", line_name, stations)
